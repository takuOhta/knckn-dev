 ...
 */
class HundredBoxes extends BaseThreeCanvas {
  ...
  constructor({ canvasElement }: { canvasElement: HTMLCanvasElement }) {
    ...
    console.log("HundredBoxes", this.#canvasElement);
  }
  ...
  #initComputeRenderer() {
    ...
    this.gpuComputeRenderer.setVariableDependencies(this.#gpuComputePositionVariables as Variable, [
      this.#gpuComputePositionVariables as Variable,
    ]);
    this.gpuComputeRenderer.setVariableDependencies(this.#gpuComputeUvVariables as Variable, [
      this.#gpuComputeUvVariables as Variable,
    ]);
  ...
  #initPositionTexture() {
    ...
    this.#gpuComputePositionVariables = this.gpuComputeRenderer.addVariable(
      "texturePosition",
  ...
  #initParticlesMesh() {
    ...
    const vertice = box.attributes.position.clone();
    geometry.setAttribute("position", vertice);
    const uv = box.attributes.uv.clone();
    geometry.setAttribute("uv", uv);
    const indices = box.index.clone();
  ...
  render({ time }: { time: number }) {
    ...
    // カメラをY軸で回転させる
    const radius = 12000;
    this._camera.position.x = Math.cos(time * 0.5) * radius;
    this._camera.position.z = Math.sin(time * 0.5) * radius;
    ...
    // テクスチャユニフォームする
    this.#particleUniforms.texturePosition.value = this.gpuComputeRenderer.getCurrentRenderTarget(
      this.#gpuComputePositionVariables as Variable
    ).texture;
    this.#particleUniforms.textureUv.value = this.gpuComputeRenderer.getCurrentRenderTarget(
      this.#gpuComputeUvVariables as Variable
    ).texture;
    this.#particleUniforms.uTexture.value = this.#texture;
